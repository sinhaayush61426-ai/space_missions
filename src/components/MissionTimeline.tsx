...
const MissionTimeline = ({ missions, planetColor }: MissionTimelineProps) => {
  ...
  return (
    <div className="relative">
      {/* Timeline line */}
      <div 
        className="absolute left-4 sm:left-8 top-0 bottom-0 w-0.5 opacity-30"
      ...
      <div className="space-y-8">
        {missions.map((mission, index) => {
          ...
          return (
            <div 
              key={mission.name}
              className="relative pl-10 sm:pl-20 opacity-0 animate-fade-in"
              style={{ animationDelay: `${index * 0.1}s` }}
            >
              {/* Timeline dot */}
              <div 
                className="absolute left-2 sm:left-6 w-4 h-4 sm:w-5 sm:h-5 rounded-full border-4 border-background"
                style={{ backgroundColor: planetColor }}
              />

              <article className="bg-card border border-border/50 rounded-xl p-4 sm:p-6 hover:border-primary/30 transition-all duration-300">
                <div className="flex flex-col sm:flex-row sm:flex-wrap sm:items-start sm:justify-between gap-2 sm:gap-4 mb-4">
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-sm">
                      ...
                      <span className="text-muted-foreground text-xs sm:text-sm">{mission.agency}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3 sm:gap-4">
                    ...
                    <div className={`flex items-center gap-1 text-xs sm:text-sm ${statusConfig[mission.status].color}`}>
                ...
                <div className="bg-secondary/30 rounded-lg p-3 sm:p-4">
                  <h4 className="font-display text-xs uppercase tracking-wider text-primary mb-2 sm:mb-3">
                    Key Achievements
                  </h4>
